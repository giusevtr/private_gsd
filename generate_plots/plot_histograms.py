...
def custom_plot(x,  **kwargs):
    ...
    if kwargs['label'] == real_data_label:
        kwargs['color'] = 'k'
        sns.histplot(data=x, bins=BINS, binrange=(0, brange),  stat='density', alpha=1.0, fill=False, hatch='/',  **kwargs)
    else:
        sns.histplot(data=x, bins=BINS, binrange=(0, brange), stat='density', **kwargs)
