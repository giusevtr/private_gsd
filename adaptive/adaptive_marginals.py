...
def worst_approximated(workload_answers, data_sync: Dataset, workload, eps, penalty=True):
    ...
    for cl in workload:
        ...
        errors = np.append(errors, max_error)
...
def adaptive(get_generator, data, epsilon, seed, delta=1e-5, workload=None, rounds=None, alpha=0.5, data_size=500):
    ...
    for i in range(1, rounds+1):
        ...
        start_errors = jnp.abs(x - init_sync_ans)
        start_error = np.linalg.norm(start_errors, ord=1)
        print('Round', i, 'of', rounds, 'Selected', ax, f': start round l1-error {start_error:.3f}, max-error={start_errors.max():.3f}', end=' ')
