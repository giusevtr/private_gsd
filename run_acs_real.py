...
def run_acs_real(gen_name, queries, epsilon_list: list, seed_list: list,
    ...
    modules = {
        'Halfspaces':
            Halfspace.get_kway_random_halfspaces(data.domain, k=1, rng=jax.random.PRNGKey(0), random_hs=50000)[0],
        'Prefix': Prefix.get_kway_prefixes(data.domain, k=1, rng=jax.random.PRNGKey(0), random_prefixes=50000)[0],
        'Ranges':
            Marginals.get_all_kway_mixed_combinations(data.domain, k_disc=1, k_real=2, bins=[2, 4, 8, 16, 32, 64])[0],
    ...
    Results_df.to_csv(f'acsreal_{gen_name}_{queries}_results_{seed_list[0]}.csv', index=False)
