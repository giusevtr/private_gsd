...
class GeneticSDConsistent(Generator):
    ...
    def __init__(self,
        ...
        self.domain = domain
        self.data_size = data_size
        if round_data_size > self.data_size:
            self.round_data_size = data_size
        else:
        ...
        self.strategy = SimpleGAforSyncData(domain,
                                            round_data_size, population_size=population_size,
    ...
    def fit(self, key, adaptive_statistic: ChainedStatistics,
        ...
        def fitness_fn(stats: chex.Array, violations: chex.Array, weight: chex.Array, pop_state: PopulationState):
            ...
            fitness = jnp.linalg.norm(selected_noised_statistics - upt_sync_stat / self.round_data_size, ord=2) ** 2
            fitness = fitness + jnp.dot(upt_vio / self.round_data_size, weight)
            return fitness
        ...
        X_sync_all = jnp.array([]).reshape((0, len(self.domain)))
        ...
        for i in range(self.round):
            ...
            if sync_dataset is not None and self.round_data_size * (i + 1) <= self.data_size:
                temp = init_sync[:, self.round_data_size * i, self.round_data_size * (i + 1)].reshape(
                    (1, init_sync.shape[0], init_sync.shape[1]))
            ...
            elite_stat = self.round_data_size * statistics_fn(state.best_member)  # Statistics of best SD
            elite_violations = self.round_data_size * self.inconsistency_fn(state.best_member)  # Statistics of best SD
            ...
            for t in range(self.num_generations):
                ...
                # UPDATE elite_states
                t0 = timer()
                elite_stat, elite_violations = update_elite_stat_jit(elite_stat, elite_violations, population_state,
                                                                     rep_best, best_id)
                ...
                if (t % self.stop_early_min_generation) == 0 and t > self.stop_early_min_generation:
                    ...
                    if loss_change < 0.0001:
                        ...
                        if self.print_progress:
                            print(
                                f'\t\tUpdating consistency weight: t={t:>5}, W.max()={W.max():.4f}, W.mean()={W.mean():.4f}')
            ...
            X_sync = state.best_member
            X_sync_all = jnp.concatenate([X_sync_all, X_sync], axis=0)
        X_sync_all = jax.random.permutation(key, X_sync_all, independent=True)
        X_sync_all = X_sync_all[:self.data_size]

        sync_dataset = Dataset.from_numpy_to_dataset(self.domain, X_sync_all)
        return sync_dataset
