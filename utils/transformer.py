...
class DataTransformer(object):
    ...
    def fit(self, data, discrete_columns=tuple(), real_columns=tuple()):
        self.output_info = []
        ...
        for column in data.columns:
            ...
            domain_attrs.append(meta['name'])
            domain_shape.append(meta['output_dimensions'])
        return Domain(domain_attrs, domain_shape)
    ...
    def transform_ordinal(self, data_df):
        ...
        for meta in self.meta:
            ...
            column_data = data_df[[meta['name']]].values
            enc = preprocessing.OrdinalEncoder()
            enc.fit(column_data)
            column_data_trans = enc.transform(column_data)
            values.append(column_data_trans)

        data_np = np.concatenate(values, axis=1).astype(float)
        return data_np
