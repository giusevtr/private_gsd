...
class Halfspace(Marginals):
    ...
    def fit(self, data: Dataset):
        ...
        # for idx in self.marginal_idx:
            ...
            for i in range(self.num_hs_samples):
    ...
    def get_halfspace_marginal_stats_fn_helper(self, cols: tuple, keys: chex.PRNGKey):
        ...
        def stat_fn(X, key):
            ...
            hs_mat = numeric_dim * 2 * (jax.random.uniform(rng_h, shape=(numeric_dim, 1)) - 0.5)   # d x h
        ...
        stat_fn_vmap = jax.vmap(stat_fn, in_axes=(None, 0))
        if num_hs_queries == 1:
    ...
    def get_sync_data_errors(self, X):
        ...
        for i in range(len(self.halfpaces_stats)):
            ...
            max_error = jnp.abs(self.halfpaces_stats[i] - sync_stat).max(axis=1)
...
def test_hs():
    ...
    # cols = []
    key = jax.random.PRNGKey(0)
    stat_mod = Halfspace(domain, kway_combinations=cols, rng=key, num_random_halfspaces=10)
    stat_mod.fit(data)
    ...
    print(stat_mod.get_sync_data_errors(rand_data.to_numpy()))
...
def test_fit_runtime():
    ...
    t0 = time.time()
    key = jax.random.PRNGKey(0)
    stat_mod, _ = Halfspace.get_kway_random_halfspaces(domain, k=1, rng=key, random_hs=10000)
    stat_mod.fit(data)
    t1 = time.time()
