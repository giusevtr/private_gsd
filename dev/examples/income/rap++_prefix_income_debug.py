...
if __name__ == "__main__":
    ...
    # module1 = PrefixDiff(data.domain, k_cat=0, cat_kway_combinations=[], rng=key, k_prefix=2, num_random_prefixes=50000)
    module1 = PrefixDiff.get_kway_prefixes(data.domain, k_cat=0, k_num=2, rng=key, random_prefixes=50000)
    ...
    for lr in [0.00009]:
        ...
        algo = RelaxedProjectionPP(domain=data.domain, data_size=1000, iterations=10000, learning_rate=[lr], print_progress=True)
        ...
        for eps in [1.00]:
            for seed in [0]:
                for rounds in [50]:
                    ...
                    t0 = timer()
                    sync_dir = f'sync_data_debug/{dataset_name}/RAP++/Prefix/{rounds}/{num_sample}/{eps:.2f}/'
                    ...
                    print(f'debug RAP++({dataset_name}, lr={lr}): eps={eps:.2f}, seed={seed}'
