...
from stats import ChainedStatistics, Halfspace, HalfspaceDiff, MarginalsDiff
...
if __name__ == "__main__":
    ...
    print(f'debug1')
    module_prefix = Halfspace.get_kway_random_halfspaces(data.domain, k=0, rng=key, random_hs=250)
    module1 = HalfspaceDiff.get_kway_random_halfspaces(data.domain, k=0, rng=key, random_hs=250)

    prefix_diff_fn = module1._get_dataset_statistics_fn()
    prefix_fn = module_prefix._get_dataset_statistics_fn()

    ans1 = prefix_diff_fn(data)
    ans2 = prefix_fn(data)

    print(jnp.abs(ans1 - ans2).max())
