...
def get_consistency_fn(domain, preprocessor):
    ...
    def row_inconsistency(x: jnp.ndarray):
        ...
        for indp_code, indp_cat_str in INDP_CODES.items():
            indp_code_encoded = get_encoded_value("INDP", int(indp_code))
            indp_cat = INDP_CAT[indp_cat_str]
            indp_cat_code_encoded = get_encoded_value("INDP_CAT", indp_cat)
            temp = (x[indp_idx] == indp_code_encoded) & (x[indp_cat_idx] != indp_cat_code_encoded)
