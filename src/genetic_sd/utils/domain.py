...
class ColumnAttribute:
    ...
    # Optional
    HAS_NULLS = 'has_nulls'
    NULL_RATE = 'null_rate'  # Frequency of nulls entries in a column
    IS_SPARSE = 'is_sparse'
    SPARSE_VALUE = 'sparse_value'  # This is the must common value found in the sparse column
    SPARSE_RATE = 'sparse_rate'  # Frequency of the SPARSE_VALUE in the sparse column
    BIN_EDGES = 'bin_edges'  # Frequency of the SPARSE_VALUE in the sparse column
...
class Domain:
    ...
    def get_continuous_cols(self):
        n_cols = []
        for c in self.attrs:
            if self.config[c][ColumnAttribute.TYPE] in ('float', 'continuous'):
                n_cols.append(c)
        return n_cols
    ...
    def get_ordinal_cols(self):
        n_cols = []
        for c in self.attrs:
            if self.config[c][ColumnAttribute.TYPE] in ('int', 'ordinal'):
                n_cols.append(c)
        return n_cols
    ...
    def get_categorical_cols(self):
        c_cols = []
        for c in self.attrs:
            if self.config[c][ColumnAttribute.TYPE] in ('string', 'categorical'):
                c_cols.append(c)
        return c_cols

    def is_continuous(self, c):
        return self.config[c][ColumnAttribute.TYPE] in ('float', 'continuous')

    def is_categorical(self, c):
        return self.config[c][ColumnAttribute.TYPE] in ('string', 'categorical')

    def is_ordinal(self, c):
        return self.config[c][ColumnAttribute.TYPE] in ('int', 'ordinal')
    ...
    def range(self, att):
        if self.is_continuous(att):
            return (0, 1)
        return (0, self.size(att))

    def type(self, att):
        return self.config[att][ColumnAttribute.TYPE]
